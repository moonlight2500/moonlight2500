...
- 기록 저장: `db.py` 가 `state/daytrader.db`(SQLite, WAL)를 관리한다. 거래(trades), 손익 곡선(equity), 판단 일지(journal), 주문 원장(order_intents), 시장 평가, 백테스트 기록, 경고 이상 로그(app_log)가 여기 있다. `db_import.py` 는 옛 JSONL을 한 번 옮긴다. 백업은 `python run.py db-backup`.
- 조건부 오버나잇: `engine._settle_overnight`(장 마감 때 판단), `_rearm_carried_oco`(다음 날 서버 OCO 재등록), `config.exit.overnight_*`.
...
- 기록은 `db.py` 로만 남긴다. 새 JSONL 기록 파일을 만들지 않는다. 쿼리는 항상 파라미터 바인딩을 쓴다.
- 버전은 `daytrader/__init__.py` 의 `__version__` 한 곳에서 관리한다. 올릴 때는 `CHANGELOG.md` 맨 위에 항목을 추가한다(앱의 릴리즈 노트 화면이 이 파일을 읽는다).
- `build_exe.bat` 은 CP949 + CRLF 로 저장한다(`.gitattributes` 참고). UTF-8로 저장하면 한글이 깨진다.
