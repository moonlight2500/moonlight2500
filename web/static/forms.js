  ...
  const CONFIG_SCHEMA = [
    {
      ...
      fields: [
        { path: "mode", label: "국내주식 사용", kind: "select",
          options: [
          ...
          hint: "★ 국내주식은 다른 시장처럼 끌 수 없는 대신, '관찰(web)'로 두면 사실상 매매를 안 하는 것과 같습니다(신호만 보고 주문은 안 냄) - 해외주식·암호화폐·스윙의 '사용' 스위치와 같은 자리이되, 껐다 켰다 대신 모드로 고릅니다. 실제 조정은 [국내주식] → '동작 모드'에서도 똑같이 할 수 있습니다(같은 값)." },
    ...
    },
    {
      ...
      note: "이 모드는 국내주식에만 적용됩니다 - 암호화폐는 왼쪽 '암호화폐' 그룹의 '암호화폐 거래 모드'로 완전히 따로 켭니다.",
      fields: [
        {
    ...
    },
    {
      ...
      note: "이 프로그램에서 가장 중요한 부분입니다. 암호화폐·해외주식의 손절·익절·추적청산·최대보유시간·일일 최대 거래 횟수는 각자 별도 값(왼쪽 '암호화폐'·'해외주식' 그룹의 '위험 관리' 섹션)을 씁니다 - 변동성이 달라 같은 기준을 못 씁니다. 다만 아래 '연속 손절 한도·재진입 금지 시간·재개 간격'은 해외주식에도 그대로 적용됩니다(해외주식은 별도 값이 없습니다).",
      fields: [
        {
    ...
    // ★★★ "해외주식/암호화폐/스윙 탭에 들어가면 좌측에 메뉴로 분류되지 않았다" - _cfgSectionShell()
    // 은 한 그룹 안에 섹션이 2개 이상이어야 좌측(데스크톱)/칩(모바일) 섹션 목록을 보여준다.
    // 예전엔 이 세 시장이 각각 섹션 하나에 모든 필드를 몰아 둬서 국내주식만 그 목록이 보였다 -
    // 아래처럼 같은 group 안에서 title 이 다른 여러 섹션으로 나눠 국내주식과 같은 대접을 준다.
    {
      title: "암호화폐 - 기본", group: "암호화폐",
    ...
    {
      title: "해외주식 - 기본", group: "해외주식",
      note: "국내주식과 같은 매매 기법·같은 계좌를 씁니다(위 '국내주식' 탭의 진입·청산 '기법 선택'을 그대로 따릅니다). "
        + "손절·익절·추적청산 폭·최대 보유시간은 위험 관리 섹션에서 따로 정합니다(원화·달러, 변동성이 다른 시장이라 국내와 분리 - "
        + "기본값은 국내와 동일하게 시작합니다).",
      fields: [
    ...
    {
      title: "해외주식 - 종목 선정", group: "해외주식",
      note: "종목(테마 없이 관심종목만 봄)·실적 기록·미국 정규장 시간을 정합니다.",
      fields: [
    ...
    {
      title: "스윙 - 위험 관리", group: "스윙",
      note: "투자금액은 세 시장 통화를 환산 없이 그대로 섞어 씁니다(대략적인 예산 관리용).",
      fields: [
